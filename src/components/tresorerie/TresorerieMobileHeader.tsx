...
interface TresorerieMobileHeaderProps {
  stats: {
    soldeQonto: string
    revenusMois: string
    depensesEnRetard?: number
  }
  onSearchClick?: () => void
  toolbar?: ReactNode
...
export function TresorerieMobileHeader({
  stats,
  onSearchClick,
  toolbar,
  headerActions,
  showGlobalNav = true,
}: TresorerieMobileHeaderProps) {
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base font-semibold text-white truncate">Trésorerie</h1>
              <p className="text-[10px] text-white/60 truncate">
                Solde {stats.soldeQonto} • Rev. {stats.revenusMois}
                {stats.depensesEnRetard && stats.depensesEnRetard > 0 && (
                  <span className="text-red-300"> • {stats.depensesEnRetard} retard</span>
                )}
              </p>
            </div>
          </div>
