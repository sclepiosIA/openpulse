...
import { fetchLatestPresence, upsertPresence } from '@/services/pulse/presenceService'
...
export function StatusSelector() {
  ...
  // Charger l'état de présence initial
  useEffect(() => {
    if (!currentProfile?.id) return

    const loadPresence = async () => {
      const data = await fetchLatestPresence(currentProfile.id)
      if (data) {
        setPresenceState({
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            className="h-7 sm:h-8 gap-1 sm:gap-1.5 px-1.5 sm:px-2 max-w-full"
          >
            <span className="flex items-center gap-1 sm:gap-1.5 min-w-0">
              ...
              {presenceState.isAutomatic && (
                <Calendar className="h-3 w-3 text-muted-foreground flex-shrink-0 hidden sm:block" />
              )}
            </span>
