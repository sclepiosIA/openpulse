// Smoke auth tests for rgpd-export-data
// Vérifie que la fonction refuse les appels non authentifiés
// (export de données personnelles RGPD = jamais d'accès anonyme).
...
test("rgpd-export-data - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/rgpd-export-data`, { method: "OPTIONS" });
  await res.text();
  assertEquals(res.status, 200);
});

test("rgpd-export-data - rejects unauthenticated POST", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/rgpd-export-data`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "apikey": ANON },
    body: JSON.stringify({}),
  });
  await res.text();
  // Anonyme = jamais 200 (RGPD : on n'exporte que pour le user authentifié)
  assertEquals(res.status >= 400, true);
});

test("rgpd-export-data - rejects forged JWT", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/rgpd-export-data`, {
    method: "POST",
    headers: {
