...
test("generate-ai-suggestions - returns 401 without service role", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/generate-ai-suggestions`, {
    method: "POST",
    headers: {
...
test("generate-ai-suggestions - returns 400 for invalid input", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/generate-ai-suggestions`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    ...
    body: JSON.stringify({ thread_id: "not-a-uuid" }),
  });
  const body = await res.text();
  // Should be 400 (invalid input) or 401 (auth required first)
  assertEquals(res.status === 400 || res.status === 401, true);
});

test("generate-ai-suggestions - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/generate-ai-suggestions`, {
    method: "OPTIONS",
  });
  await res.text();
