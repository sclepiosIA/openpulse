// Smoke security tests for signature-cancel
...
test("signature-cancel - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/signature-cancel`, { method: "OPTIONS" });
  await res.text();
  assertEquals(res.status, 200);
});

test("signature-cancel - rejects unauthenticated POST", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/signature-cancel`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "apikey": ANON },
    body: JSON.stringify({ signature_request_id: "00000000-0000-0000-0000-000000000000" }),
  });
  await res.text();
