// Smoke security tests for signature-send (DocuSeal pipeline)
...
test("signature-send - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/signature-send`, { method: "OPTIONS" });
  await res.text();
  assertEquals(res.status, 200);
});

test("signature-send - rejects unauthenticated POST", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/signature-send`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "apikey": ANON },
    body: JSON.stringify({ contrat_id: "00000000-0000-0000-0000-000000000000" }),
  });
  await res.text();
  assertEquals(res.status >= 400, true);
});

test("signature-send - rejects forged JWT", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/signature-send`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "apikey": ANON, "Authorization": "Bearer forged.jwt.token" },
    body: JSON.stringify({}),
  });
  await res.text();
  assertEquals(res.status >= 400, true);
