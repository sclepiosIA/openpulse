...
test("send-email-reply - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/send-email-reply`, {
    method: "OPTIONS",
  });
  await res.text();
...
test("send-email-reply - returns 401 without auth", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/send-email-reply`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ thread_id: "test", content: "test" }),
  });
  const body = await res.text();
  assertEquals(res.status, 401);
});

test("send-email-reply - returns error for missing fields", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/send-email-reply`, {
    method: "POST",
    headers: {
