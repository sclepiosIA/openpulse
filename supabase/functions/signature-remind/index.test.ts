// Smoke security tests for signature-remind
...
test("signature-remind - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/signature-remind`, { method: "OPTIONS" });
  await res.text();
  assertEquals(res.status, 200);
});

test("signature-remind - rejects unauthenticated POST", async () => {
