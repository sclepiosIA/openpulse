...
test("rd-ai-assist - returns 400 without titre", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/rd-ai-assist`, {
    method: "POST",
    headers: {
...
test("rd-ai-assist - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/rd-ai-assist`, {
    method: "OPTIONS",
  });
  await res.text();
