// Smoke security tests for qonto-webhook-handler
...
test("qonto-webhook-handler - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/qonto-webhook-handler`, { method: "OPTIONS" });
  await res.text();
  assertEquals(res.status, 200);
});

test("qonto-webhook-handler - rejects without HMAC signature", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/qonto-webhook-handler`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${ANON}`, "apikey": ANON },
    body: JSON.stringify({ event: "transaction.created", data: {} }),
  });
  await res.text();
  assertEquals(res.status >= 400, true);
});

test("qonto-webhook-handler - rejects forged HMAC signature", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/qonto-webhook-handler`, {
    method: "POST",
    headers: {
