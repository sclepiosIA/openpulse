...
Deno.test('source keeps the chat query table allowlist and AI sanitization', async () => {
  const source = await Deno.readTextFile(new URL('./index.ts', import.meta.url))

  assertEquals(source.includes('WHITELISTED_TABLES'), true)
  assertEquals(source.includes('sanitizeForAI'), true)
  assertEquals(source.includes('execute_readonly_query'), true)
})
