// Smoke security tests for facturation-actions (mutations factures/avoirs)
...
test("facturation-actions - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/facturation-actions`, { method: "OPTIONS" });
  await res.text();
  assertEquals(res.status, 200);
});

test("facturation-actions - rejects unauthenticated POST", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/facturation-actions`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "apikey": ANON },
    body: JSON.stringify({ action: "create_facture" }),
  });
  await res.text();
  assertEquals(res.status >= 400, true);
});

test("facturation-actions - rejects unknown action", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/facturation-actions`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "apikey": ANON, "Authorization": `Bearer ${ANON}` },
    body: JSON.stringify({ action: "__not_a_real_action__" }),
  });
  await res.text();
  assertEquals(res.status >= 400, true);
});
