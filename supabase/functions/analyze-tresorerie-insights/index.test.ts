...
Deno.test('source preserves finance authorization and protected treasury context', async () => {
  const source = await Deno.readTextFile(new URL('./index.ts', import.meta.url))

  assertEquals(source.includes('validateServiceOrUser(req)'), true)
  assertEquals(source.includes("['admin', 'direction'].includes(r.role)"), true)
  assertEquals(source.includes("wrapUserContent(dataContext, 'TREASURY_DATA')"), true)
})
