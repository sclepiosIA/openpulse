...
Deno.test('source retains email transfer limits and CORS policy', async () => {
  const source = await Deno.readTextFile(new URL('./index.ts', import.meta.url))

  assertEquals(source.includes('MAX_FILES = 20'), true)
  assertEquals(source.includes('MAX_ACTIVE_TRANSFERS = 10'), true)
  assertEquals(source.includes('Access-Control-Allow-Methods'), true)
})
