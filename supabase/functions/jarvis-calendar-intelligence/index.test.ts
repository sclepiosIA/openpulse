...
Deno.test('keeps authenticated-user and event-ownership guards', async () => {
  const source = await Deno.readTextFile(new URL('./index.ts', import.meta.url))

  assertStringIncludes(source, "claimsData.claims.role !== 'authenticated'")
  assertStringIncludes(source, 'const userId = authenticatedUserId')
  assertStringIncludes(source, ".eq('calendars.owner_id', userId)")
})
