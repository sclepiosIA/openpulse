...
test("correct-spelling-email - returns 400 without text", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/correct-spelling-email`, {
    method: "POST",
    headers: {
...
test("correct-spelling-email - CORS preflight works", async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/correct-spelling-email`, {
    method: "OPTIONS",
  });
  await res.text();
