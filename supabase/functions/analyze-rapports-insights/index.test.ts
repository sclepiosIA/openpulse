...
Deno.test(
  'source retains authenticated insight analysis and controlled error responses',
  async () => {
    const source = await Deno.readTextFile(new URL('./index.ts', import.meta.url))

    assertEquals(source.includes('Missing authorization header'), true)
    assertEquals(source.includes('response_format: { type: "json_object" }'), true)
    assertEquals(source.includes("buildErrorResponse('analyze-rapports-insights'"), true)
  }
)
