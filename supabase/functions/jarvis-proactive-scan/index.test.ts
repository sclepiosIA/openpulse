...
Deno.test('keeps the CRON-only internal-secret gate', async () => {
  ...
  assertStringIncludes(source, 'const denied = requireInternalSecret(req, corsHeaders)')
  assertStringIncludes(source, 'if (denied) return denied')
})
